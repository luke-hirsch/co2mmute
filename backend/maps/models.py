...
class Edge(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        dx = (
            self.node1.x_coord - self.to_node.x_coord
        )  # was is to_node? meinst du node2?
        dy = (
            self.node1.y_coord - self.to_node.y_coord
        )  # was is to_node? meinst du node2?
    ...
    def __str__(self):
        return f"Edge belonging to {self.game_map}. Edge is of type {self.edgetype_choices} running from {self.node1} to {self.to_node}"  # was is to_node? meinst du node2?
...
class TrainTrackEdge(Edge):
    ...
    def __str__(self):
        return f"Train Track Edge from {self.node1} to {self.node2} with {self.number_of_tracks} tracks"  # number of tracks fehlt im model
